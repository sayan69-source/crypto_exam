...
export default function SetterIRTPage() {
  ...
  return (
    <div style={{ animation: 'fadeIn 300ms ease forwards' }}>
      <h1 style={{ fontSize: 24, color: '#fffefb', marginBottom: 8 }}>IRT Parameter Editor</h1>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
        {[
          ...
          <div key={v.label} style={{ padding: '8px 16px', borderRadius: 12, background: v.ok ? 'rgba(111, 166, 120,0.1)' : 'rgba(194, 90, 72,0.1)', border: `1px solid ${v.ok ? 'rgba(111, 166, 120,0.2)' : 'rgba(194, 90, 72,0.2)'}`, fontSize: 12, fontWeight: 500, color: v.ok ? '#6fa678' : '#c25a48' }}>
            {v.ok ? '✓' : '✗'} {v.label}
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 16 }}>
        {/* Question list */}
        <div style={{ background: 'rgba(255, 254, 251,0.03)', border: '1px solid var(--color-navy-700)', borderRadius: 16, overflow: 'hidden', maxHeight: 500, overflowY: 'auto' }}>
          {questions.map(q => (
            <div key={q.id} style={{ padding: '10px 14px', borderBottom: '1px solid rgba(255, 254, 251,0.04)', display: 'flex', justifyContent: 'space-between', cursor: 'pointer', transition: 'background 150ms' }}>
        ...
        {/* Scatter placeholder */}
        <div style={{ background: 'rgba(255, 254, 251,0.03)', border: '1px solid var(--color-navy-700)', borderRadius: 16, padding: 24, position: 'relative', minHeight: 400 }}>
          <h3 style={{ fontSize: 14, color: 'var(--color-navy-200)', marginBottom: 16 }}>Difficulty vs. Discrimination</h3>
          <div style={{ position: 'relative', width: '100%', height: 320, background: 'rgba(255, 254, 251,0.02)', borderRadius: 8, border: '1px dashed var(--color-navy-600)' }}>
            {/* Axis labels */}
            ...
            {questions.map(q => (
              <div key={q.id} style={{
                ...
                background: q.subject === 'Physics' ? '#939084' : q.subject === 'Chemistry' ? '#939084' : '#6fa678',
                transform: 'translate(-50%, 50%)', cursor: 'pointer',
                boxShadow: `0 0 6px ${q.subject === 'Physics' ? 'rgba(147, 144, 132,0.4)' : q.subject === 'Chemistry' ? 'rgba(147, 144, 132,0.4)' : 'rgba(111, 166, 120,0.4)'}`,
              }} title={`Q${q.sequence_number}: b=${q.irt_b}, a=${q.irt_a}`} />
            ))}
            {/* Target zone */}
            <div style={{ position: 'absolute', left: '35%', right: '35%', bottom: '15%', top: '40%', background: 'rgba(96, 93, 82,0.08)', border: '1px dashed var(--color-navy-500)', borderRadius: 8 }}>
