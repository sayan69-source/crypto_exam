...
# veritysetup (static), the /init script, and nothing else — no shells on the
# final rootfs means the initramfs is the only place busybox ever exists, and
# it is discarded at switch_root.
set -euo pipefail
...
[zuup-os] Build-host artifact; not running here. On the build host it would:
            1. stage busybox.static and veritysetup.static (build-host verified)
...
BUSYBOX="${1:?usage: mkinitramfs.sh <busybox.static> <veritysetup.static>}"
VERITYSETUP="${2:?missing veritysetup.static}"
